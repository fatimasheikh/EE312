...
int hammingDistance(int aList[], int bList[], int len) {
  int hamDist = 0;
  for(int i = 0; i < len; i++) {
    if(aList[i] != bList[i]) {   //compare each index value
      hamDist++;
    }
  }
  return hamDist;
...
bool lastDigit(int x, int y) {
  ...
  if (xLastDigit == yLastDigit) {   //compare last digit
    return true;
  }
  return false;
}
...
int sum3or5Multiples() {
  ...
  for(int num = 1; num < 1000; num ++) {
    ...
    else if ((num%5) == 0) { //if not multiple of 3, check 5
      sum = sum + num;
    }
  }
  return sum;
...
int reverseInt(int x) {
  ...
  while (x != 0) {
    int digit = x%10;
    reverse = (reverse*10)+digit;   //shift last digit to left and add new digit
    x = (x-digit)/10;
    if (llabs(reverse) > INT_MAX) {  //test for integer overflow
      return 0;
    }
  }
  return reverse;
